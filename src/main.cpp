...
#if defined(NDEBUG)
# error "Rosscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x73af9804efba3769063192addb871fff72c92cdcfa51a3940c67120ebc92917a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rosscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rosscoin Signed Message:\n";
...
{
    // Rosscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Rosscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Rosscoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Rosscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rosscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rosscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd608aab0dd8dcc3c584ea95bf1d167941a86686b4666ee789cc89abb29ab3cb0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "De Dion Graus-Tapes 13/2/2021";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bf0ea1bed5c2feb475a24fce00eecae51467c9f668ca23ecfe1e8bdfa6cda6b7302816b731487a1728962e65f12b968f9211ed9231981c78ae02fbb00cf39f21") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1613241739;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084944149;

        if (fTestNet)
        {
            block.nTime    = 1613241702;
            block.nNonce   = 385999362;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xff45c8d6364a5febc900e58198a59bbe4c5b4ca6579105013372c510d106e821"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xbb, 0xd3 }; // Rosscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RosscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RosscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RosscoinMiner(CWallet *pwallet)
{
    printf("RosscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rosscoin-miner");
    ...
    try { loop {
        ...
        printf("Running RosscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RosscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RosscoinMiner, pwallet));
}
